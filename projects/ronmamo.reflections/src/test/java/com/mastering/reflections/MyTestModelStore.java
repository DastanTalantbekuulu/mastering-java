//generated using Reflections JavaCodeSerializer [Sat Sep 04 04:04:04 JST 2021]
package com.mastering.reflections;

public interface MyTestModelStore {

  interface com {
    interface mastering {interface reflections {
      interface TestModel$AC1 {
      }
      interface TestModel$AC1n {
      }
      interface TestModel$AC2 {
        interface methods {
          interface value {}
        }
      }
      interface TestModel$AC3 {
        interface annotations {
          interface com_mastering_reflections_TestModel$AC2 {}
        }
      }
      interface TestModel$AF1 {
        interface methods {
          interface value {}
        }
      }
      interface TestModel$AI1 {
        interface annotations {
          interface com_mastering_reflections_TestModel$MAI1 {}
        }
      }
      interface TestModel$AI2 {
      }
      interface TestModel$AM1 {
        interface methods {
          interface value {}
        }
      }
      interface TestModel$AM2 {
      }
      interface TestModel$C1 {
        interface annotations {
          interface com_mastering_reflections_TestModel$AC1 {}
          interface com_mastering_reflections_TestModel$AC1n {}
        }
      }
      interface TestModel$C2 {
        interface annotations {
          interface com_mastering_reflections_TestModel$AC2 {}
        }
      }
      interface TestModel$C3 {
        interface annotations {
          interface com_mastering_reflections_TestModel$AC2 {}
        }
      }
      interface TestModel$C4 {
        interface fields {
          interface f1 {}
          interface f2 {}
          interface f3 {}
        }
        interface methods {
          interface add {}
          interface c2toC3 {}
          interface m1 {}
          interface m1_int$$$$__java_lang_String$$$$ {}
          interface m3 {}
          interface m4 {}
        }
      }
      interface TestModel$C5 {
      }
      interface TestModel$C6 {
      }
      interface TestModel$C7 {
        interface annotations {
          interface com_mastering_reflections_TestModel$AC3 {}
        }
      }
      interface TestModel$I1 {
        interface annotations {
          interface com_mastering_reflections_TestModel$AI1 {}
        }
      }
      interface TestModel$I2 {
        interface annotations {
          interface com_mastering_reflections_TestModel$AI2 {}
        }
      }
      interface TestModel$I3 {
        interface annotations {
          interface com_mastering_reflections_TestModel$AC2 {}
        }
      }
      interface TestModel$MAI1 {
      }
    }}
  }
}
