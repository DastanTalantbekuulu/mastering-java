...
public class Box extends Mobile {
    ...
    private static int getRGB(char c) {
        ...
        if ('0' <= c && c <= '9') {
            int code = c - '0';
            rgb = switch (code) {
                case 1 -> color1.getRGB();
                case 2 -> color2.getRGB();
                case 6 -> color6.getRGB();
                case 7 -> color7.getRGB();
                case 8 -> color8.getRGB();
                case 9 -> color9.getRGB();
                default -> color0.getRGB();
            };
        }
        return rgb;
    }
    ...
    public Box get(int number) {
        resetVisited();
        return getInternal(number);
    }

    private Box getInternal(int number) {
        ...
        if (next != null && !next.visited) {
            return next.getInternal(number);
        }
        if (top != null && !top.visited) {
            return top.getInternal(number);
        }
        return null;
    }
