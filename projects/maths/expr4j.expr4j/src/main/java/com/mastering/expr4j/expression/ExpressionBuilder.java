...
public class ExpressionBuilder<T> {
    ...
    private boolean formTree(ExpressionNode node, Token token) {
        if (node.token() instanceof Function) {
            ...
            if (!node.children().isEmpty() && formTree(node.children().getFirst(), token)) {
                return true;
            } else if (node.children().size() < operandCount) {
                node.children().addFirst(new ExpressionNode(token));
                return true;
            }
    ...
    private void formTree(Stack<Token> postfix) {
        while (!postfix.isEmpty()) {
            ...
            if (expression.getRoot() == null) {
                expression.setRoot(new ExpressionNode(token));
            } else {
                boolean flag = formTree(expression.getRoot(), token);
    ...
    public Expression<T> build(String expr) {
        try {
            ...
            expression = new Expression<T>(expressionDictionary, expressionConfig);
