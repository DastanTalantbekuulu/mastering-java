public class Model {
    ...
    public void doAction(String command) {
        if(!expression.isEmpty() && Character.isLetter(expression.charAt(0))){
            expression = "";
        }
        switch (command) {
            case "Clear":
                ...
                break;
            case "Equal":
                expression = rpn.calculate(expression);
                break;
            case "ToggleSign":
    ...
    private String processCommand(String command, String expression) {
        ...
        if (command.equals("0") && expression.equals("0")) {
            return expression;
        }
    ...
    private String handleBracket(String expression) {
        if (expression.isEmpty()) {
            return "(";
        } else if ((hasOpenBrackets(expression) &&
                    !lastCharIsOperator(expression)) &&
                    !isAllOpenBracket(expression)) {
            return expression + ")";
        } else {
            return expression + "(";
        }
    ...
    private boolean isAllOpenBracket(String expression) {
