...
package org.examples.orthography;
...
public class BismillahExample {
	...
	public static void main() {
		...
		for (Sura sura : Quran.getSuras()) {
			...
			if (sura.getBismillah() == null) {
				System.out.println("No bismillah: " + sura);
			}

			// Otherwise, check if the chapter has a shadda
			// on the first letter, the Ba.
			else if (sura.getBismillah().getCharacter(0).isShadda()) {
				System.out.println("Shadda on Ba: " + sura);
			}
		}

		// Step #2. Search for bismillah in verses.
		for (Aya aya : Quran.getAyas()) {
			if (aya.toBuckwalter().contains(
					"bisomi {ll~ahi {lr~aHoma`ni {lr~aHiymi")) {
				System.out.println("Bismillah in verse " + aya.getLocation());
			}
		}
	}
}
