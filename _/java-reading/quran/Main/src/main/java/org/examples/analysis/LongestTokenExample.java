...
public class LongestTokenExample {
	...
	public static void main() {
		...
		AnalysisTable tokenTable = new AnalysisTable("SuraNumber",
				"AyaNumber", "TokenNumber", "Token");
		for (Token token : Quran.getTokens()) {
			if (token.getLetterCount() == maxTokenLength) {
				tokenTable.add(token.getSuraNumber(),
						token.getAyaNumber(), token.getTokenNumber(), token
