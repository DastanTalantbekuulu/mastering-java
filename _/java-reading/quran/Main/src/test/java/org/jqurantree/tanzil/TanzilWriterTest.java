...
public class TanzilWriterTest {
	...
	private void writeChapter(Sura sura) {

		// Start.
		validateLine("\t<sura index=\""
				+ sura.getSuraNumber()
				+ "\" name=\""
				+ sura.getName().toString(EncodingType.Unicode,
		...
		for (Aya aya : sura) {
			...
			text.append(aya.getAyaNumber());
			text.append("\" text=\"");
			text.append(aya.toUnicode());
			text.append("\"");

			// Bismillah.
			if (aya.getAyaNumber() == 1 && sura.getBismillah() != null) {
				text.append(" bismillah=\"");
				text.append(sura.getBismillah());
				text.append("\"");
			}
			text.append(" />");
